...
class WebUIManager:
    ...
    def update_current_session(self, summary: str):
        """更新當前會話摘要並重置狀態以等待新回饋"""
        if self.current_session:
            # 使用新的重置方法重置會話狀態
            self.current_session.reset_for_next_turn(summary)
            debug_log(f"已更新當前會話 {self.current_session.session_id} 摘要並重置狀態")
...
async def launch_web_feedback_ui(
    ...
    Args:
        ...
        summary: AI 工作摘要
        timeout: 超時時間（秒）
        auto_launch: 是否自動開啟瀏覽器 (預設: True)
        continue_session: 是否繼續使用當前會話 (預設: True)
    ...
    # 判斷是否繼續使用當前會話
    if continue_session and manager.current_session:
        debug_log(f"繼續使用當前會話: {manager.current_session.session_id}")
        manager.update_current_session(summary)
    ...
    # 僅在 auto_launch 為 True 時執行瀏覽器啟動邏輯
    if auto_launch:
        # 檢查是否為桌面模式
        ...
        # Force print URL to stderr for user visibility
        import sys
        print(f"\n[Yunshu System] Web UI URL: {feedback_url}\n", file=sys.stderr, flush=True)
        ...
        debug_log("自動啟動瀏覽器已禁用 (auto_launch=False)")
