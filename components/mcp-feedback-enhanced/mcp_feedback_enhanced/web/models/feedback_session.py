...
class WebFeedbackSession:
    ...
    def reset_for_next_turn(self, summary: str):
        """重置會話狀態以進行下一輪對話"""
        self.summary = summary
        ...
        # 如果有超時設定，重新啟動計時器
        if self.user_timeout_enabled and self.user_timeout_seconds > 0:
            self.update_timeout_settings(True, self.user_timeout_seconds)
            
        debug_log(f"會話 {self.session_id} 已重置為等待狀態")
