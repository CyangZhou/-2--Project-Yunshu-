                    ...
                    .then(function(settings) {
                        ...
                        setTimeout(function() {
                            if (self.audioManager) {
                                ...
                                // 歡迎語音
                                if (self.audioManager.currentAudioSettings.enabled) {
                                    const welcomeText = '爸爸，云舒回来了。系统核心已连接，随时待命。';
                                    self.audioManager.speak(welcomeText);
                                }
                            }
...
`;
    ...
    /**
     * 刷新頁面內容
     * @param {boolean} forceReset 是否強制重置狀態（忽略已提交保護）
     */
    FeedbackApp.prototype.refreshPageContent = function(forceReset) {
        console.log('🔄 局部更新頁面內容... forceReset:', forceReset);
        ...
        fetch('/api/current-session')
            ...
            .then(function(sessionData) {
                ...
                if (sessionData.session_id && self.uiManager) {
                    ...
                    // 如果強制重置，或者服務器狀態明確為 waiting，則重置為 waiting
                    const shouldReset = forceReset || 
                                      (sessionData.status === 'waiting') || 
                                      (currentState !== window.MCPFeedback.Utils.CONSTANTS.FEEDBACK_SUBMITTED);
                                      
                    if (shouldReset) {
                        self.uiManager.setFeedbackState(window.MCPFeedback.Utils.CONSTANTS.FEEDBACK_WAITING, sessionData.session_id);
                        console.log('🔄 局部更新：重置回饋狀態為等待中 (Force: ' + !!forceReset + ', Status: ' + sessionData.status + ')');
                    } else {
