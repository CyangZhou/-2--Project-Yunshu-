...
(function() {
     ...
     * 載入可用語音
     */
    AudioManager.prototype.loadVoices = function() {
        this.voices = this.synth.getVoices();
        // 優先選擇中文語音
        this.preferredVoice = this.voices.find(voice => voice.lang.includes('zh') && voice.name.includes('Google')) ||
                              this.voices.find(voice => voice.lang.includes('zh')) ||
                              this.voices[0];
        console.log('🎤 已載入語音列表，偏好語音:', this.preferredVoice ? this.preferredVoice.name : 'None');
    };
    ...
    AudioManager.prototype.speak = function(text) {
        if (!this.currentAudioSettings.enabled) return;
        ...
        utterThis.volume = this.currentAudioSettings.volume / 100;
        
        // 解決某些瀏覽器語音合成被過早垃圾回收的問題
        window.speechSynthesisUtteranceReference = utterThis;

        utterThis.onend = function() {
            delete window.speechSynthesisUtteranceReference;
        };

        this.synth.speak(utterThis);
        console.log('🗣️ 正在朗讀:', text, '語音:', this.preferredVoice ? this.preferredVoice.name : 'Default');
    };
