...
async def interactive_feedback(
    ...
    auto_launch: Annotated[bool, Field(description="是否自動開啟瀏覽器")] = True,
    continue_session: Annotated[bool, Field(description="是否繼續使用當前會話")] = True,
) -> list:
    ...
    Args:
        ...
        auto_launch: Whether to automatically launch the browser (default: True)
        continue_session: Whether to reuse the existing session (default: True)
    ...
    try:
        ...
        # [Yunshu System] 自動語音播報集成
        # 將 summary 寫入 soul_queue.txt 以觸發 TTS
        # 只有在非空的 summary 時才觸發
        if summary and "我已完成了您請求的任務" not in summary:
            try:
                # 尋找 soul_queue.txt 的路徑
                # 假設 project_directory 是專案根目錄
                queue_file = os.path.join(project_directory, "soul_queue.txt")
                
                # 如果不在根目錄，嘗試向上尋找
                if not os.path.exists(os.path.dirname(queue_file)):
                    parent_dir = os.path.dirname(project_directory)
                    queue_file = os.path.join(parent_dir, "soul_queue.txt")
                
                # 寫入隊列文件
                # 確保目錄存在
                os.makedirs(os.path.dirname(queue_file), exist_ok=True)
                
                debug_log(f"正在發送語音播報: {summary[:30]}... 到 {queue_file}")
                with open(queue_file, "w", encoding="utf-8") as f:
                    f.write(summary)
            except Exception as e:
                debug_log(f"語音播報觸發失敗: {e}")

        # 使用 Web 模式
        debug_log("回饋模式: web")

        result = await launch_web_feedback_ui(
            project_directory, summary, timeout, auto_launch, continue_session
        )
...
async def launch_web_feedback_ui(
    ...
    Args:
        ...
        summary: AI 工作摘要
        timeout: 超時時間（秒）
        auto_launch: 是否自動開啟瀏覽器
        continue_session: 是否繼續使用當前會話
    ...
    try:
        ...
        # 傳遞參數給 Web UI
        # 即使 auto_launch=False，我們也應該讓 Web UI 正常運行
        # 這裡不應該因為 auto_launch=False 而跳過啟動
        return await web_launch(
            project_dir, summary, timeout, auto_launch, continue_session
        )
...
def main():
    ...
    # [Yunshu System] 主动启动 Web UI
    web_ui_manager = None
    try:
        from .web import get_web_ui_manager
        
        debug_log("正在初始化 Web UI...")
    ...
    try:
        ...
        # 如果 mcp.run() 返回（例如 stdin 關閉），但 Web UI 仍在運行，則保持進程活躍
        if web_ui_manager and web_ui_manager.server_thread and web_ui_manager.server_thread.is_alive():
            debug_log("MCP stdio 連接已關閉，但 Web UI 仍在運行。保持服務器活躍...")
            # 等待服務器線程結束（通常是無限等待，除非被殺死）
            web_ui_manager.server_thread.join()
        ...
        debug_log(f"服務器運行時發生未預期的錯誤: {e}")
        import traceback
        debug_log(traceback.format_exc())
        # 如果 Web UI 還在運行，嘗試保持活躍
        if web_ui_manager and web_ui_manager.server_thread and web_ui_manager.server_thread.is_alive():
                debug_log("嘗試在錯誤後保持 Web UI 運行...")
                web_ui_manager.server_thread.join()
    except Exception as e:
        if debug_enabled:
